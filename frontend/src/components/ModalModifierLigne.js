...
export default function ModalModifierLigne({ ligne, onClose, onSaved }) {
  ...
  const allPoints = [
    ...new Set(segments.flatMap((s) => [s.point_a, s.point_b]).filter(Boolean)),
  ];
  ...
  const ajouterSegment = () => {
    const dernierB =
      segments.length > 0 ? segments[segments.length - 1].point_b : "";
    setSegments([
      ...segments,
      { point_a: dernierB, point_b: "", prix_normal: "" },
    ]);
  };

  const supprimerSegment = (i) =>
  ...
  const valider = () => {
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      const doublon = segments.some(
        (o, j) => j !== i && o.point_a === s.point_a && o.point_b === s.point_b,
      );
      if (doublon)
        return `Segment ${i + 1} : doublon (${s.point_a} → ${s.point_b}).`;
    }
    return null;
  };
  ...
  const handleSave = async () => {
    ...
    setError("");
    try {
      await axios.put(`/api/lignes/${ligne.id_ligne}`, {
        nom_ligne: nomLigne,
          ...
          {segments.map((seg, i) => (
            <div
              ...
              style={{
                display: "grid",
                gridTemplateColumns:
                  "24px 1fr 24px 1fr 120px 90px 90px auto auto",
                alignItems: "center",
                gap: 8,
            ...
            >
              ...
              <span style={{ textAlign: "center", color: "var(--gray-400)" }}>
                →
              </span>
              ...
              <input
                ...
                step="100"
                onChange={(e) =>
                  updateSegment(i, "prix_normal", e.target.value)
                }
              />

              {/* Prix calculés */}
              <span
                className="badge-role direction"
                style={{ textAlign: "center", fontSize: "0.78rem" }}
              >
                {seg.prix_normal ? (seg.prix_normal * 0.5).toFixed(3) : "—"}
              </span>
              <span
                className="badge-role red"
                style={{ textAlign: "center", fontSize: "0.78rem" }}
              >
