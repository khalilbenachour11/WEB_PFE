...
const fmtTime = (d) => {
  if (!d) return "—";
  const local =
    typeof d === "string" ? d.replace("T", " ").replace("Z", "") : d;
...
function ModalTransactions({ journee, onClose }) {
  const { data, loading, erreur } = useModalData(
    `/controleur/tickets`,
    journee,
  );
  ...
  const qtePayants = filtered.reduce(
    (s, t) => (Number(t.montant_total) > 0 ? s + Number(t.quantite || 0) : s),
    0,
  );
  const qteGratuits = filtered.reduce(
    (s, t) => (Number(t.montant_total) === 0 ? s + Number(t.quantite || 0) : s),
    0,
  );
  ...
  const handleExport = () => {
    ...
    XLSX.writeFile(
      wb,
      `transactions_${journee.matricule_agent}_${normalizeDate(journee.date)}.xlsx`,
    );
  };
      ...
      >
        <div className="modal-header">
          <div>
            ...
            >
              {fmtDate(journee.date)} ·{" "}
              {journee.nom_ligne || `Ligne #${journee.id_ligne}`}
            </div>
          </div>
        ...
        >
          ...
          <div className="ctrl-modal-kpi-item">
            ...
            <div className="ctrl-modal-kpi-value green">
              {fmt(totalPayant)} ms
            </div>
          </div>
        </div>
                  ...
                  return (
                    <tr key={t.id_ticket}>
                      ...
                      <td style={{ fontSize: "0.87rem" }}>
                        ...
                        <span
                          style={{ color: "var(--gray-400)", margin: "0 6px" }}
                        >
                          →
                        </span>
                      ...
                      <td>
                        <span
                          className={
                            isGratuit ? "badge-tarif-free" : "badge-tarif-paid"
                          }
                        >
                          {t.type_tarif}
                        </span>
...
function ModalRapportDetail({ journee, onClose }) {
  const {
    ...
    loading,
    erreur,
  } = useModalData(`/controleur/rapport_detail`, journee);
  ...
  const tauxGrat =
    r.nb_tickets > 0
      ...
      : "0.0";
  const prixMoyen =
    r.nb_payants > 0 ? Math.round(Number(r.total_ms) / r.nb_payants) : 0;
  ...
  const handleExport = () => {
    ...
    XLSX.writeFile(
      wb,
      `rapport_${journee.matricule_agent}_${normalizeDate(journee.date)}.xlsx`,
    );
  };
      ...
      >
        <div className="modal-header">
          <div>
            ...
            >
              {fmtDate(journee.date)} ·{" "}
              {journee.nom_ligne || `Ligne #${journee.id_ligne}`} · Matricule{" "}
              {journee.matricule_agent}
            </div>
          </div>
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            <button
        ...
        <div style={{ padding: "20px 28px" }}>
          ...
          <div className="ctrl-rapport-kpi-grid">
            ...
            <div className="ctrl-rapport-kpi-item navy">
              ...
              <div className="ctrl-rk-sub">
                {r.nb_payants} payants · {r.nb_gratuits} gratuits
              </div>
            </div>
                ...
                <tbody>
                  {r.par_tarif.map((t) => {
                    const pct =
                      Number(r.total_ms) > 0
                        ? (
                            (Number(t.total) / Number(r.total_ms)) *
                            100
                          ).toFixed(1)
                        : "0.0";
                    return (
                ...
                <tbody>
                  {r.par_voyage.map((v) => (
                    <tr key={v.id_voyage}>
                      ...
                      <td>
                        <span
                          className={`badge-role ${v.type === "programmé" ? "informatique" : "direction"}`}
                        >
                          {v.type}
                        </span>
                      </td>
                      <td style={{ fontSize: "0.87rem" }}>
                        {v.nom_ligne || `Ligne ${v.id_ligne}`}
                      </td>
          ...
          {journee.sync_failed > 0 && (
            <div className="ctrl-anomalie-banner">
              ...
              <div>
                <div className="ctrl-anomalie-title">
                  Anomalie de synchronisation détectée
                </div>
                <div className="ctrl-anomalie-desc">
                  {journee.sync_failed} ticket(s) n'ont pas été synchronisés
                  correctement depuis l'application mobile.
                </div>
              </div>
            </div>
          )}
...
export default function ControleurRecettes() {
  ...
  const fetchJournees = useCallback(async () => {
    setLoading(true);
    try {
      const res = await axios.get(`/controleur/journees`, {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchReceveur, filterLigne, dateDebut, dateFin]);

  // ── Filtres client ──────────────────────────────────────────────────────────
  const lignesUniques = [
    ...new Set(journees.map((j) => j.nom_ligne).filter(Boolean)),
  ].sort();
  ...
  const totalRecette = filtered.reduce(
    (s, j) => s + Number(j.recette_ms || 0),
    0,
  );
  const totalTickets = filtered.reduce(
    (s, j) => s + Number(j.nb_tickets || 0),
    0,
  );
  const totalPayants = filtered.reduce(
    (s, j) => s + Number(j.nb_payants || 0),
    0,
  );
  const totalGratuits = filtered.reduce(
    (s, j) => s + Number(j.nb_gratuits || 0),
    0,
  );
    ...
    <div>
      ...
      <div className="card ctrl-filter-card">
        ...
        <p className="ctrl-filter-info">
          ...
          <strong>{fmtDate(dateDebut)}</strong> au{" "}
          <strong>{fmtDate(dateFin)}</strong>
        </p>
      </div>
