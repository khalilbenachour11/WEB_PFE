    ...
    <div className="modal-overlay">
      <div className="modal-box">
        ...
        <div className="modal-message">
          ...
          <br />
          Rôle web :{" "}
          <strong>{roleLabels[agent.role_web] || agent.role_web}</strong>
        </div>
...
export default function GestionMdp_info() {
  ...
  const fetchUtilisateurs = async () => {
    try {
      const res = await axios.get(`/utilisateurs_web_mdp`);
  ...
  return (
    <div>
      ...
      <div className="breadcrumb">
        SRTB › Informatique › <span>Gestion des mots de passe</span>
      </div>
      ...
      <div className="card">
          ...
          <tbody>
            {currentItems.map((u) => (
              <tr key={u.matricule_agent}>
                <td>
                  <span className="badge-matricule">{u.matricule_agent}</span>
                </td>
                <td>{u.nom || "—"}</td>
                <td>{u.prenom || "—"}</td>
                <td>
                  <span
                    className={`badge-role ${roleColors[u.role_web] || "receveur"}`}
                  >
                ...
                <td>
                  {u.role_web === "informatique" ? (
                    <span style={{ color: "#8A94A6", fontSize: "0.8rem" }}>
                      protégé{" "}
                    </span>
                  ) : (
                    <button
        ...
        {filtered.length > 0 && (
          ...
          >
            {indexOfFirstItem + 1}–{Math.min(indexOfLastItem, filtered.length)}{" "}
