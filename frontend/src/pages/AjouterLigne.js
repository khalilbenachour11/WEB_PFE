...
function SelectAgence({ value, onSelect }) {
  ...
  const handleChange = (e) => {
    ...
    if (!code) {
      onSelect({ code_agence: "", nom_agence: "" });
      return;
    }
  ...
  return (
    ...
    >
      <option value="">
        {loading ? "Chargement des agences..." : "Sélectionner une agence "}
      </option>
      {agences.map((ag) => (
  ...
  return (
    <div>
      ...
      >
        ...
        <button className="btn segment-add-btn" onClick={ajouter}>
          ➕ Ajouter
        </button>
      </div>
      ...
      {arrets.length > 0 ? (
        ...
        <div className="receveur-empty">
          Aucun arrêt défini — ajoutez les arrêts dans l'ordre du trajet
        </div>
      )}
...
function TableauSegments({ segments, points, onUpdate, onDelete }) {
  ...
  if (!segments.length)
    return <div className="receveur-empty">Aucun segment ajouté</div>;
...
export default function AjouterLigne() {
  ...
  const pointsAller = [
    ...new Set(aller.flatMap((s) => [s.point_a, s.point_b])),
  ];
  const pointsRetour = [
    ...new Set(retour.flatMap((s) => [s.point_a, s.point_b])),
  ];
  ...
  const handleSubmit = async () => {
    ...
    if (!aller.length)
      return setMessage({
        text: "Ajoutez au moins un segment aller.",
        type: "error",
      });

    // ✅ Auto-compléter les arrêts si pas encore définis manuellement
    const arretsAllerFinal =
      arretsAller.length >= 2 ? arretsAller : autoDetectArrets(aller);
    const arretsRetourFinal =
      arretsRetour.length >= 2 ? arretsRetour : autoDetectArrets(retour);
    ...
    try {
      ...
      if (retour.length) {
        const r2 = await axios.post(`${API}/ajouter_ligne`, {
          ...
          segments: retour,
          arrets_ordre: arretsRetourFinal,
        });
        if (!r2.data.success)
          return setMessage({
            text: `Retour : ${r2.data.message}`,
            type: "error",
          });
      }
      ...
      setMessage({
        text: retour.length
          ? "Lignes aller et retour ajoutées !"
          : "Ligne aller ajoutée !",
        type: "success",
      });
      setForm(EMPTY_FORM);
  ...
  return (
    <div>
      <div className="breadcrumb">
        SRTB › Direction › <span>Ajouter une ligne</span>
      </div>
      <div className="page-header">
        <div>
          <div className="page-title">Ajouter une ligne</div>
          <div className="page-subtitle">
            Définir la ligne, ses segments aller/retour et leurs tarifs
          </div>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="form-group">
          ...
          <input
            className="form-input"
            placeholder="Ex: Bizerte-Tunis"
            value={form.nom_ligne}
            onChange={(e) =>
              setForm((f) => ({ ...f, nom_ligne: e.target.value }))
            }
          />
        </div>
        <div className="ligne-grid-2">
          <div className="form-group">
            ...
            <input
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, point_depart: e.target.value }))
              }
            />
          </div>
          <div className="form-group">
            ...
            <input
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, point_arrive: e.target.value }))
              }
            />
          </div>
        </div>
        <div className="form-group">
          ...
          <SelectAgence
            ...
            onSelect={(ag) => {
              setForm((f) => ({
                ...f,
                code_agence: ag.code_agence ? String(ag.code_agence) : "",
              }));
      ...
      <div className="card">
        <div className="form-section-title">
          ...
          <span className="segment-count">
            ({aller.length} segment{aller.length !== 1 ? "s" : ""})
          </span>
        </div>
        <FormulaireSegment
          newSeg={newAller}
          setNewSeg={setNewAller}
          points={pointsAller}
          error={errors.aller}
      ...
      <div className="card">
        <div className="form-section-title">
          ...
          <span className="segment-count">
            ({arretsAller.length} arrêt{arretsAller.length !== 1 ? "s" : ""})
          </span>
        </div>
        <p
          style={{
        ...
        >
          Définissez l'ordre exact des arrêts. C'est cet ordre qui sera utilisé
          pour ouvrir/clôturer les segments lors du voyage.
        </p>
        <OrdreArrets
          arrets={arretsAller}
          setArrets={setArretsAller}
          pointsSuggestions={pointsAller}
          error={errors.arretsAller}
        />
      </div>
      ...
      <div className="card">
        <div className="section-header">
          <div className="form-section-title">
            ...
            <span className="segment-count">
              ({retour.length} segment{retour.length !== 1 ? "s" : ""})
            </span>
          </div>
          <button className="btn btn-generer" onClick={genererRetour}>
            ⇄ Générer depuis l'aller
          </button>
        </div>
        <FormulaireSegment
          newSeg={newRetour}
          setNewSeg={setNewRetour}
          points={pointsRetour}
          error={errors.retour}
          onAjouter={() =>
            ajouterSegment(newRetour, setRetour, retour, "retour")
          }
        />
        <TableauSegments
          segments={retour}
      ...
      {retour.length > 0 && (
        <div className="card">
          <div className="form-section-title">
            ...
            <span className="segment-count">
              ({arretsRetour.length} arrêt{arretsRetour.length !== 1 ? "s" : ""}
              )
            </span>
          </div>
          <p
          ...
          <OrdreArrets
            arrets={arretsRetour}
            setArrets={setArretsRetour}
            pointsSuggestions={pointsRetour}
            error={errors.arretsRetour}
          />
        </div>
      )}
