            ...
            style={{
              ...
              background:
                page === currentPage
                  ? "#e8f0fe"
                  : "var(--color-background-primary)",
              color: page === currentPage ? "#1a73e8" : "inherit",
...
function ModalRole({ agent, onClose, onSuccess, isBootstrap }) {
...
export default function GestionRoles({ user }) {
  ...
  const isBootstrap = user?.matricule === 0;
  ...
  const getRoleWeb = (matricule) =>
    utilisateurs.find((u) => u.matricule_agent === matricule);
  ...
  return (
    <div>
      ...
      <div className="breadcrumb">
        SRTB › Informatique › <span>Gestion des rôles</span>
      </div>
      <div className="page-header">
        <div>
          <div className="page-title">Gestion des rôles web</div>
          <div className="page-subtitle">
            {utilisateurs.length} utilisateur(s) avec rôle défini
          </div>
        </div>
      </div>
              ...
              return (
                <tr key={agent.matricule_agent}>
                  ...
                  <td>
                    {userWeb ? (
                      <span
                        className={`badge-role ${roleColors[userWeb.role_web]}`}
                      >
                        {roleLabels[userWeb.role_web]}
                      </span>
                    ) : (
                  ...
                  <td>
                    {userWeb ? formatDateOnly(userWeb.date_attribution) : "—"}
                  </td>
                  <td style={{ display: "flex", gap: 8 }}>
                    {(!estInformatique || isBootstrap) && (
                      <button
                    ...
                    {userWeb && !estInformatique && (
                      <button
                        ...
                        onClick={() => handleDelete(agent.matricule_agent)}
                      >
                        Retirer
                      </button>
                    )}
