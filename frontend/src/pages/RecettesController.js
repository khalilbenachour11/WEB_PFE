...
const fmtDate = (str) => str ? new Date(str).toLocaleDateString("fr-FR") : "—";
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, marginBottom: 24 }}>
      {cards.map((c) => (
        <div key={c.label} style={{
          ...
          <div style={{ fontSize: "0.75rem", color: "#8A94A6", marginBottom: 4 }}>{c.label}</div>
          <div style={{
...
function PeriodFilter({ startDate, endDate, onStart, onEnd, onReset }) {
  ...
  return (
    <div style={{
      ...
      <div>
        <div style={{ fontSize: "0.75rem", color: "#8A94A6", marginBottom: 4 }}>Date début</div>
        <input type="date" className="form-input"
          value={startDate} max={endDate || undefined}
          onChange={(e) => onStart(e.target.value)} />
      </div>
      <div>
        <div style={{ fontSize: "0.75rem", color: "#8A94A6", marginBottom: 4 }}>Date fin</div>
        <input type="date" className="form-input"
      ...
      {active && (
        <>
          <button className="btn btn-gray" onClick={onReset} style={{ alignSelf: "flex-end" }}>
            ✕ Réinitialiser
          </button>
          <span style={{ alignSelf: "flex-end", fontSize: "0.8rem", color: "#2563EB", fontWeight: 600, paddingBottom: 8 }}>
            {startDate && endDate
              ? `${startDate} → ${endDate}`
...
function ChartCard({ title, children }) {
  return (
    <div style={{
      ...
      <div style={{ fontWeight: 700, fontSize: "0.9rem", color: "#1A2332", marginBottom: 16 }}>
        {title}
      </div>
      {children}
    </div>
...
function Empty() {
  return (
    <div style={{ textAlign: "center", color: "#8A94A6", padding: "40px 0", fontSize: "0.88rem" }}>
...
export default function RecettesController() {
  ...
  const fetchData = async (start = "", end = "") => {
    try {
      ...
      axios.get(`${API}/journal_ventes`, { params })
        .then((res) => setJournal(res.data?.data   || []))
        .catch(()   => setJournal([]));

      axios.get(`${API}/anomalies_recettes`, { params })
  ...
  useEffect(() => { fetchData(); },                    []);
  useEffect(() => { fetchData(startDate, endDate); }, [startDate, endDate]);
  ...
  const evolutionByDay = useMemo(() => {
    ...
    return Object.keys(map).sort().map((date) => ({ date, recette: map[date] }));
  }, [filtered]);
  ...
  if (loading) {
    return (
      <div className="page-container">
        ...
        <div style={{ color: "#8A94A6", marginTop: 40, textAlign: "center" }}>Chargement…</div>
      </div>
    );
  }
  ...
  return (
    <div className="page-container">
      ...
      <div className="breadcrumb">SRTB › <span>Contrôleur des recettes</span></div>
      ...
      <KpiStrip total={totalRecette} nbVoyages={filtered.length} nbAnomalies={anomalies.length} />

      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>

        <ChartCard title="📊 Recettes par ligne">
          {revenueByLine.length === 0 ? <Empty /> : (
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={revenueByLine} margin={{ bottom: 30 }}>
                <XAxis dataKey="ligne" tick={{ fontSize: 10 }} angle={-20} textAnchor="end" interval={0} />
          ...
          {evolutionByDay.length === 0 ? <Empty /> : (
            <ResponsiveContainer width="100%" height={280}>
              <LineChart data={evolutionByDay}>
                ...
                <Line type="monotone" dataKey="recette" stroke="#28B463"
                  strokeWidth={2} dot={evolutionByDay.length <= 31} activeDot={{ r: 5 }} />
              </LineChart>
            </ResponsiveContainer>
          )}
        </ChartCard>
        ...
        {top5.length === 0 ? <Empty /> : (
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={top5} layout="vertical" margin={{ left: 10, right: 30 }}>
              ...
              <YAxis type="category" dataKey="ligne" tick={{ fontSize: 11 }} width={110} />
              <Tooltip formatter={fmtDT} />
      ...
      <ChartCard title="⚠ Anomalies de recettes">
        {anomalies.length === 0 ? (
          <div style={{ color: "#1B6B3A", fontSize: "0.88rem" }}>✓ Aucune anomalie détectée</div>
        ) : (
          <table className="recettes-table">
            <thead>
              <tr>
                <th>Voyage</th><th>Ligne</th><th>Receveur</th><th>Date</th><th>Écart</th>
              </tr>
            </thead>
            <tbody>
              {anomalies.map((a) => (
                <tr key={a.id_vente}>
                  ...
                  <td style={{ fontSize: "0.82rem" }}>{fmtDate(a.date_heure)}</td>
                  <td style={{ color: "#C0392B", fontWeight: 700 }}>{fmtDT(a.ecart)}</td>
                </tr>
              ))}
            </tbody>
          </table>
