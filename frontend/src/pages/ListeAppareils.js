  ...
  return (
    <div
      style={{
        ...
        opacity: visible ? 1 : 0,
        transition:
          "transform 0.35s cubic-bezier(.4,0,.2,1), opacity 0.35s ease",
...
function ModalAttribution({ appareil, agents, onClose, onSuccess }) {
  ...
  const handleSubmit = async () => {
    if (!matricule_agent) {
      setError("Veuillez choisir un agent.");
      return;
    }
    setLoading(true);
    try {
      const res = await axios.put(`/attribuer_appareil/${appareil.num_serie}`, {
        matricule_agent,
      });
      if (res.data.success) {
        onSuccess();
    ...
    <div className="modal-overlay">
      <div className="modal-box">
        ...
        <div className="modal-message">
          Appareil N° <strong>{appareil.num_serie}</strong>
        </div>
        {error && (
          <div
            style={{
...
function ModalModifier({ appareil, onClose, onSuccess }) {
  ...
  useEffect(() => {
    ...
    axios
      .get(`/agents`)
      .then((res) => {
        const receveurs = (res.data.agents || []).filter(
          (a) => a.role === "receveur",
        );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const r1 = await axios.put(
        `/modifier_appareil/${appareil.num_serie}`,
        form,
      );
      if (!r1.data.success) {
        setError(r1.data.message || "Erreur modification appareil");
        setLoading(false);
        return;
      }
      const newNumSerie = form.num_serie;
      if (agentMode === "remove" && hasAgent) {
        await axios.put(`/modifier_statut/${newNumSerie}`, {
          statut: "en stocke",
          liberer: true,
        });
        ...
        await axios.put(`/attribuer_appareil/${newNumSerie}`, {
          matricule_agent: newMatricule,
        });
      }
      onSuccess();
      onClose();
      ...
      <div className="modal-box" style={{ maxWidth: 480 }}>
        ...
        <div className="modal-form">
          ...
          <div className="form-group">
            ...
            <input
              ...
              onChange={(e) =>
                setForm({ ...form, date_de_mise_en_service: e.target.value })
              }
            />
          </div>
          <div
            ...
            {hasAgent ? (
              ...
              >
                <span className="badge-matricule">
                  {appareil.matricule_agent}
                </span>
                <span>
                  {appareil.prenom} {appareil.nom}
                </span>
              </div>
            ) : (
              <div
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              >
                <input
                  ...
                  name="agentMode"
                  value="keep"
                  checked={agentMode === "keep"}
                  onChange={() => setAgentMode("keep")}
                />
              ...
              >
                <input
                  ...
                  name="agentMode"
                  value="change"
                  checked={agentMode === "change"}
                  onChange={() => setAgentMode("change")}
                />
                Changer de receveur
              </label>
              {hasAgent && (
                ...
                >
                  <input
                    ...
                    name="agentMode"
                    value="remove"
                    checked={agentMode === "remove"}
                    onChange={() => setAgentMode("remove")}
                  />
                  <span style={{ color: "#c0392b" }}>
                    Retirer le receveur (libérer l'appareil)
                  </span>
                </label>
              )}
            </div>
              ...
              {loadingAgents ? (
                ...
                >
                  ...
                  {availableAgents.map((a) => (
                    <option
                      key={a.matricule_agent}
                      value={a.matricule_agent}
                      style={
                        String(a.matricule_agent) ===
                        String(appareil.matricule_agent)
                          ? { fontWeight: "bold" }
                          : {}
                      }
                    >
          ...
          {agentMode === "remove" && (
            ...
            >
              ⚠ Le receveur sera libéré et l'appareil passera en statut{" "}
        ...
        <div className="modal-actions">
          ...
          <button
            ...
            disabled={loading || (agentMode === "change" && !newMatricule)}
          >
...
export default function ListeAppareils() {
  ...
  const fetchAll = async () => {
    try {
      const r = await axios.get(`/appareils`);
      setAppareils(r.data.appareils || []);
    } catch {
      setAppareils([]);
    }
    try {
      const r = await axios.get(`/agents_libres`);
  ...
  const handleStatut = async (app, statutCible) => {
    try {
      const res = await axios.put(`/modifier_statut/${app.num_serie}`, {
        statut: statutCible,
        liberer: app.statut === "actif",
      });
      if (res.data.success) {
        showMessage("Statut modifié avec succès.", "success");
        fetchAll();
  ...
  return (
    <div>
      ...
      {appareilAttribution && (
        <ModalAttribution
          appareil={appareilAttribution}
          agents={agents}
          onClose={() => setAppareilAttribution(null)}
          onSuccess={() => {
            showMessage("Appareil attribué avec succès.", "success");
            fetchAll();
          }}
        />
      )}
      {appareilModifier && (
        <ModalModifier
          ...
          onSuccess={() => {
            showMessage("Appareil modifié avec succès.", "success");
            fetchAll();
          }}
        />
      )}
      ...
      <div className="breadcrumb">
        SRTB › Direction › <span>Liste des appareils</span>
      </div>
      <div className="page-header">
        <div>
          <div className="page-title">Liste des appareils</div>
          <div className="page-subtitle">
            {filtered.length} appareil(s) trouvé(s) — {agents.length} agent(s)
            disponible(s)
          </div>
        </div>
        <button className="btn" onClick={() => navigate("/ajouter-appareil")}>
          ➕ Ajouter appareil
        </button>
      </div>
            ...
            {currentItems.length > 0 ? (
              currentItems.map((app) => (
                <tr key={app.num_serie}>
                  <td>
                    <span className="badge-matricule">{app.num_serie}</span>
                  </td>
                  <td>
                    <span
              ...
              <tr className="empty-row">
                <td colSpan={6}>Aucun appareil trouvé</td>
              </tr>
            )}
          </tbody>
        </table>
