...
const TYPE_OPTIONS = [
  { value: 'programmé',     label: '📅 Programmé',     desc: "Planifié à l'avance", bgLight: '#e8f0fe', border: '#1a73e8', text: '#1a73e8' },
  { value: 'non programmé', label: '⚡ Non programmé', desc: 'Immédiat / imprévu',   bgLight: '#fffbeb', border: '#f59e0b', text: '#92400e' },
];
      ...
      {TYPE_OPTIONS.map(opt => {
        ...
        return (
          <button key={opt.value} onClick={() => onChange(opt.value)} style={{
            ...
            fontWeight: 500, fontSize: 13, textAlign: 'center', transition: 'all 0.15s',
          }}>
            <div>{opt.label}</div>
...
function SegmentsVoyage({ idVente }) {
  const [segments, setSegments] = useState([]);
  const [loading,  setLoading]  = useState(true);

  useEffect(() => {
    axios.get(`${API}/segments_voyage/${idVente}`)
    ...
    <>
      <tr className="voyage-row">
        <td onClick={() => setExpanded(!expanded)}>
          <span className="badge-matricule">{voyage.id_vente}</span>
        </td>
        <td onClick={() => setExpanded(!expanded)}>
        ...
        <td>
          <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <button className="action-btn delete"
              onClick={e => { e.stopPropagation(); onDelete(voyage); }}>
              supprimer
            </button>
      ...
      {expanded && (
        <tr>
          <td colSpan={7} className="segments-row">
            <SegmentsVoyage idVente={voyage.id_vente} />
          </td>
        </tr>
      )}
    </>
...
function ModalAjouterVoyage({ matricule, appareil, onClose, onSuccess, voyages }) {
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/segments_ligne/${form.id_ligne}`)
      .then(r => setSegments(r.data.segments || [])).catch(() => setSegments([]));
  }, [form.id_ligne]);
  ...
  const handleSubmit = async () => {
    ...
    const doublon = voyages.find(v => v.id_ligne === parseInt(form.id_ligne) && v.statut !== 'cloture');
    if (doublon) return setError('Un voyage actif existe déjà pour cette ligne.');
      ...
      <div className="modal-box" style={{ maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div className="modal-form" style={{ overflowY: 'auto', flex: 1, paddingRight: 4 }}>
          ...
          <div className="form-group">
            ...
            <select className="form-input" value={form.id_ligne}
          ...
          <div className="form-group">
            ...
            <input type="datetime-local" className="form-input"
              value={form.date_heure} min={minDateStr}
...
function ModalModifierVoyage({ voyage, onClose, onSuccess }) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.put(`${API}/modifier_voyage/${voyage.id_vente}`, {
        id_ligne: form.id_ligne, type: form.type, date_heure: form.date_heure,
      });
      if (res.data.success) { onSuccess(); onClose(); }
  ...
  return (
    <div className="modal-overlay">
      <div className="modal-box" style={{ maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div className="modal-title">Modifier le voyage #{voyage.id_vente}</div>
        {error && <div className="alert alert-error">⚠ {error}</div>}
...
function ModalSupprimerVoyage({ voyage, lignesMap, onClose, onSuccess }) {
  ...
  const handleDelete = async () => {
    setLoading(true);
    try {
      const res = await axios.delete(`${API}/supprimer_voyage/${voyage.id_vente}`);
      if (res.data.success) { onSuccess(); onClose(); }
  ...
  return (
    <div className="modal-overlay">
      <div className="modal-box">
        ...
        <div className="modal-message">
          Confirmer la suppression du voyage <strong>#{voyage.id_vente}</strong> —{' '}
        ...
        <div className="modal-actions">
          <button className="btn btn-gray" onClick={onClose}>Annuler</button>
          <button className="btn" style={{ background: '#e53935', borderColor: '#e53935' }}
...
function Pagination({ currentPage, totalPages, onPageChange }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '16px', flexWrap: 'wrap' }}>
      <button onClick={() => onPageChange(currentPage - 1)} disabled={currentPage === 1}
        style={{ padding: '8px 12px', borderRadius: 4, border: '1px solid var(--color-border-tertiary)',
          background: currentPage === 1 ? 'var(--color-background-secondary)' : 'var(--color-background-primary)',
          cursor: currentPage === 1 ? 'not-allowed' : 'pointer', fontSize: 14, fontWeight: 500 }}>
        ← Précédent
      </button>
        ...
        page === '...'
          ...
          : <button key={page} onClick={() => onPageChange(page)} disabled={page === currentPage}
              style={{ width: 36, height: 36, borderRadius: 4,
                ...
                fontWeight: page === currentPage ? 600 : 500, fontSize: 13 }}>
              {page}
            </button>
      )}
      <button onClick={() => onPageChange(currentPage + 1)} disabled={currentPage === totalPages}
        style={{ padding: '8px 12px', borderRadius: 4, border: '1px solid var(--color-border-tertiary)',
          ...
          cursor: currentPage === totalPages ? 'not-allowed' : 'pointer', fontSize: 14, fontWeight: 500 }}>
        Suivant →
      </button>
    </div>
  );
...
function ModalHistoriqueVoyages({ onClose, lignesMap }) {
  ...
  useEffect(() => { fetchAllVoyages(); }, []); // eslint-disable-line
  ...
  const applyFilters = (voyages, type, statut, ligne, date, receveur) => {
    const filtered = voyages.filter(v => {
      ...
      const matchDate     = !date     || (v.date_heure && v.date_heure.startsWith(date));
      const matchReceveur = !receveur || v.nom_receveur.toLowerCase().includes(receveur.toLowerCase());
      return matchType && matchStatut && matchLigne && matchDate && matchReceveur;
    });
    setFilteredData(filtered);
    setCurrentPage(1);
  };

  const handleTypeFilter     = val => { setTypeFilter(val);     applyFilters(allVoyages, val, statutFilter, ligneFilter, dateFilter, receveurFilter); };
  const handleStatutFilter   = val => { setStatutFilter(val);   applyFilters(allVoyages, typeFilter, val, ligneFilter, dateFilter, receveurFilter); };
  const handleLigneFilter    = val => { setLigneFilter(val);    applyFilters(allVoyages, typeFilter, statutFilter, val, dateFilter, receveurFilter); };
  const handleDateFilter     = val => { setDateFilter(val);     applyFilters(allVoyages, typeFilter, statutFilter, ligneFilter, val, receveurFilter); };
  const handleReceveurFilter = val => { setReceveurFilter(val); applyFilters(allVoyages, typeFilter, statutFilter, ligneFilter, dateFilter, val); };
  ...
  return (
    <div className="modal-overlay">
      <div className="modal-box" style={{ maxHeight: '95vh', maxWidth: '95vw', width: 1300, display: 'flex', flexDirection: 'column' }}>
        <div className="modal-icon">📋</div>
        <div className="modal-title">Historique des voyages</div>

        <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--color-border-tertiary)', display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
          <div style={{ flex: '0 1 160px' }}>
            <label style={{ fontSize: 11, color: 'var(--color-text-secondary)', display: 'block', marginBottom: 4 }}>Type</label>
            <select className="form-input" value={typeFilter} onChange={e => handleTypeFilter(e.target.value)} style={{ fontSize: 13 }}>
          ...
          <div style={{ flex: '0 1 160px' }}>
            <label style={{ fontSize: 11, color: 'var(--color-text-secondary)', display: 'block', marginBottom: 4 }}>Statut</label>
            <select className="form-input" value={statutFilter} onChange={e => handleStatutFilter(e.target.value)} style={{ fontSize: 13 }}>
          ...
          <div style={{ flex: '0 1 160px' }}>
            <label style={{ fontSize: 11, color: 'var(--color-text-secondary)', display: 'block', marginBottom: 4 }}>Ligne</label>
            <select className="form-input" value={ligneFilter} onChange={e => handleLigneFilter(e.target.value)} style={{ fontSize: 13 }}>
          ...
          <div style={{ flex: '0 1 140px' }}>
            ...
            <input type="date" className="form-input" value={dateFilter} onChange={e => handleDateFilter(e.target.value)} style={{ fontSize: 13 }} />
          </div>
          <div style={{ flex: '1 1 200px' }}>
            <label style={{ fontSize: 11, color: 'var(--color-text-secondary)', display: 'block', marginBottom: 4 }}>Receveur</label>
            <input className="search-input" placeholder="Rechercher..." value={receveurFilter}
              onChange={e => handleReceveurFilter(e.target.value)} style={{ fontSize: 13 }} />
          </div>
        </div>

        <div style={{ flex: 1, overflowY: 'auto' }}>
          {loading ? (
            <div style={{ padding: 20, textAlign: 'center', color: 'var(--color-text-secondary)' }}>Chargement...</div>
          ) : filteredData.length === 0 ? (
            <div style={{ padding: 20, textAlign: 'center', color: 'var(--color-text-secondary)' }}>Aucun voyage ne correspond aux filtres</div>
          ) : (
            <table className="data-table" style={{ marginBottom: 0 }}>
              <thead>
                <tr>
                  <th>ID</th><th>Receveur</th><th>Ligne</th><th>Date / Heure</th><th>Type</th><th>Statut</th><th>Montant</th>
                </tr>
              </thead>
              <tbody>
                {paginatedData.map(v => (
                  <tr key={v.id_vente}>
                    ...
                    <td style={{ fontSize: 12 }}>{v.type === 'non programmé' ? 'Non programmé' : 'Programmé'}</td>
                    <td><span className={`badge-role ${statutColor(v.statut)}`}>{v.statut}</span></td>
        ...
        <div style={{ padding: '12px 16px', borderTop: '1px solid var(--color-border-tertiary)', fontSize: 12, color: 'var(--color-text-secondary)' }}>
          {filteredData.length === 0 ? '0 voyage(s)' : `${startIdx + 1}–${Math.min(startIdx + ITEMS_PER_PAGE, filteredData.length)} sur ${filteredData.length} total`}
        </div>
...
function ReceveurCard({ receveur, lignesMap, onVoyageAdded, onNotify, tousLesVoyages }) {
  ...
  const handleReactiver = async (voyage) => {
    try {
      const res = await axios.put(`${API}/reactiver_voyage/${voyage.id_vente}`);
      if (res.data.success) {
        onNotify({ text: `Voyage #${voyage.id_vente} réactivé avec succès !`, type: 'success' });
        onVoyageAdded();
      } else {
...
export default function ListeVoyages() {
        ...
        voyRes.data.data.forEach(row => {
          ...
          if (!grouped[key]) {
            const app = appareils.find(
              a => String(a.matricule_agent) === String(row.matricule_agent) && a.statut === 'actif'
            );
            grouped[key] = {
  ...
  const filtered = receveurs.filter(r => {
    ...
    const matchLigne = filterLigne ? r.voyages.some(v => String(v.id_ligne) === filterLigne) : true;
    const matchDate  = filterDate  ? r.voyages.some(v => v.date_heure && v.date_heure.startsWith(filterDate)) : true;
  ...
  return (
    <div>
      ...
      <div className="breadcrumb">SRTB › Direction › <span>Voyages receveurs</span></div>
      ...
      {showHistorique && (
        <ModalHistoriqueVoyages onClose={() => setShowHistorique(false)} lignesMap={lignesMap} />
      )}
      ...
      {filtered.length === 0
        ? <div className="card"><p className="text-center">Aucun receveur trouvé</p></div>
        : filtered.map(r => (
          <ReceveurCard
