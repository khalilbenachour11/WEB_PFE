...
function VoyageTimeline({ idVoyage, onClose }) {
  ...
  useEffect(() => {
    axios
      .get(`/voyage_historique/${idVoyage}`)
      ...
      >
        <div className="modal-header">
          <h2 className="modal-title">
            🕐 Historique complet — Voyage #{idVoyage}
          </h2>
        ...
        <div style={{ padding: "20px 28px" }}>
          {loading ? (
            ...
            <div className="receveur-empty">
              Aucune trace enregistrée pour ce voyage.
            </div>
          ) : (
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        <ArrowTransition
                          avant={e.statut_avant}
                          apres={e.statut_apres}
                        />
                        <span
                          style={{
                            fontSize: "0.72rem",
...
export default function HistoriqueVoyages() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [entRes, statsRes, lignesRes] = await Promise.all([
        axios.get(`/voyage_historique`, { params }),
        axios.get(`/voyage_historique_stats`, { params }),
        axios.get(`/lignes`),
      ]);
      ...
      const lMap = {};
      (lignesRes.data.lignes || []).forEach((l) => {
        lMap[l.id_ligne] = l.nom_ligne;
      });
      setLignesMap(lMap);
    } catch {
  ...
  const filtered = useMemo(() => {
    ...
    return entries.filter((e) => {
      ...
      if (filterReceveur) {
        ...
        if (!name.includes(rv) && !String(e.matricule_agent).includes(rv))
          return false;
      }
      if (q) {
  ...
  const uniqueLignes = [...new Set(entries.map((e) => e.id_ligne))].filter(
    Boolean,
  );
  const hasFilters =
    search ||
      ...
      <div className="page-header">
        <div>
          ...
          <div className="page-subtitle">
            Traçabilité complète de chaque changement de statut
          </div>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", gap: 12 }}>
          <div style={{ flex: 1, maxWidth: 280 }}>
            ...
            <input
              ...
              onChange={(e) => {
                setFilterReceveur(e.target.value);
                resetPage();
              }}
            />
          </div>
      ...
      <div className="card" style={{ padding: 0, overflow: "hidden" }}>
        {loading ? (
          ...
          <>
              ...
              <table className="data-table" style={{ margin: 0 }}>
                <thead>
                  <tr>
                    ...
                    <th style={thStyle} onClick={() => handleSort("id_voyage")}>
                      Voyage <SortIcon field="id_voyage" />
                    </th>
                    <th style={thStyle} onClick={() => handleSort("id_ligne")}>
                      Ligne <SortIcon field="id_ligne" />
                    </th>
                    <th>Receveur</th>
                ...
                <tbody>
                  {paginated.map((e) => (
                    <tr key={e.id}>
                      ...
                      >
                        {lignesMap[e.id_ligne] ||
                          (e.id_ligne ? `Ligne ${e.id_ligne}` : "—")}
                      </td>
                      <td>
                        {e.nom || e.prenom ? (
                          <div>
            ...
            >
              {startIdx + 1}–
              {Math.min(startIdx + ITEMS_PER_PAGE, sorted.length)} sur{" "}
