  ...
  for (let i = 0; i < arrets_ordre.length - 1; i++) {
    await db.promise().query(
      `INSERT INTO billetterie.segment_voyage 
       (id_vente, id_ligne, point_depart, point_arrivee, ordre, statut)
...
async function deleteLigneDetails(id_ligne) {
  await db.promise().query(
    `DELETE FROM billetterie.segment_voyage WHERE id_ligne=? AND id_vente IS NULL`, 
    [id_ligne]
  );
  await db.promise().query(
    `DELETE FROM billetterie.tarif_segment WHERE id_ligne=?`, 
    [id_ligne]
  );
